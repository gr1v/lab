...
### Временная сложность
Временная сложность — от O(n log²n) до O(n²) в зависимости от последовательности шагов.
...
Почему: в среднем случае массив делится пополам на каждом уровне рекурсии (log n уровней), на каждом уровне выполняется O(n) операций. В худшем случае (неудачный выбор опорного элемента) деление неравномерное.
